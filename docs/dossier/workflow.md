...
### Три двери из Icebox закрыты — и первый заход закрыл их НЕ ТЕМ рассуждением

Это самая полезная запись во всей секции, потому что здесь карточка ошиблась и была поймана.

Контракт-гейт SKILL.md (`test_the_rulebook_names_BOTH_stages_*`) выводит список открытых
стадий из `workflow.STAGES`, поэтому восьмая стадия сразу потребовала ответить: закрывать ли
из неё `return_task` и `decompose`, как #626 и #649 закрыли их из Done.

**Первый заход ответил НЕТ, и аргумент был такой:** карточка в Icebox без ассайни по
определению, обе двери требуют `_require_mine`, значит дотянуться можно только если человек
РУКАМИ назначил агента на замороженную карточку — а человек, который это сделал, говорит «эту
всё-таки сделай». Асимметрия с Done казалась решающей: #626 понадобился именно потому, что
обычный цикл паркует в Done НАЗНАЧЕННУЮ карточку.

**Независимое ревью построило состояние, и посылка оказалась ложной.** Перетаскивание карточки
в Vikunja НЕ трогает assignees — это ровно та причина, по которой #626 и был нужен для Done, то
есть аргумент опирался на факт, который сам себя опровергает. Обычный жизненный цикл паркует в
Icebox назначенную карточку ДВУМЯ дорогами, и обе означают противоположное «сделай»:

1. **Build → Icebox.** Человек видит, что агент жжёт токены на легаси, и замораживает на лету.
2. **Your Call → Icebox.** `call_human` СОХРАНЯЕТ ассайни, поэтому запаркованная карточка уже
   назначена, а «заморозить» — естественный ответ на вопрос. Icebox стоит справа от Done, то
   есть в одном перетаскивании от того места, куда человек и так смотрит.

Измерено из этого состояния, с Done как контролем в том же раунде: `decompose` УСПЕХ — родитель
в Backlog + `epic`, ассайни снят, **два ребёнка в Queue**, и `next_task` предложил ребёнка A
следующим же вызовом (дословно форма #649, которую репозиторий уже закрывал); `return_task`
УСПЕХ — заморозка молча откачена в Backlog + `blocked`; `transfer_task` УСПЕХ — замороженная
карточка ушла с доски в Backlog соседа. Из Done все три отказали.

**`transfer_task` — третья дверь, и её не было ни в чьём списке.** Ни в спеке, ни в worklog, ни
в этом досье до ревью. Это и решило, ГДЕ ставить гейт: не в трёх тулах, а в общей точке
`_find_task`, рядом с гейтом Done. Ровно рассуждение #662 — «новый мутирующий тул, который
двигает карточку и не проверяет стадию, открывает дыру заново», и оговорка про этот открытый
класс дословно стоит в SKILL.md. Третья дверь показала, что класс не гипотетический.

Реализация — ОТДЕЛЬНАЯ ветка, а не расширенное условие Done, по двум причинам сразу:
`test_done_is_human_only` читает AST того `BoolOp`, так что слияние молча поменяло бы предмет
того пина; и отказывают эти две стадии по РАЗНЫМ причинам, чего общее сообщение сказать не
может — Done это законченная работа, Icebox это работа, которую сознательно не начинают.
Четыре READ-пути (`get_task`, `comment`, `download_attachment`, `attach_file`) опт-инятся через
`allow_icebox=True`: замороженную карточку надо уметь читать и комментировать, потому что
именно туда и следует класть находку о ней.

**Побочное следствие: `_OWNERLESS_EXITS["Icebox"]` стал МЁРТВЫМ КОДОМ и удалён.** Тем же
механизмом, который #662 записал про свою строку для Done: гейт в `_find_task` срабатывает
раньше `_require_mine`. Измерено на этом дереве — ownerless-карточка в Icebox и все семь
ownership-гейтнутых тулов: каждый отвечает морозильным отказом, до `_require_mine` не доходит
ни один. Запись удалена, а не оставлена, по той же честности, что #662 выбрал для своих мёртвых
строк: устаревшая строка в таблице, которой читатель доверяет, хуже отсутствующей.

**Чему это учит на будущее.** Аргумент первого захода был не ленивым — он ссылался на #626 и
на реальный ключ `_require_mine`. Он был НЕПРОВЕРЕННЫМ: ни одна его посылка не была построена
на стенде, и та единственная, что решала всё («перетаскивание снимает ассайни»), опровергается
одной строкой на `FakeAPI`. Это ровно то, о чём говорит правило репозитория про гарды,
чинимые рассуждением, — и заметьте, где именно ошибка осела: не в коде, а в ПРОЗЕ, тремя
местами, где ложная посылка была напечатана как факт (`references/decompose.md`,
`references/stuck.md` и докстринг в `workflow.py`). Код без гейта просто работал; неправду
говорили комментарии.

### Кросс-проектный замороженный предшественник — та же болезнь, тот же заход

Первая версия сравнивала `pred_stage == "Icebox"` с голым литералом, тогда как резолвящая
ветка `_offboard_predecessor` рендерит стадию ДЕКОРИРОВАННОЙ: `Icebox (project 108)`. Поэтому
кросс-проектно клауза не печаталась НИКОГДА, и отказ выдавал «finish that one first» про
карточку в чужой морозилке — то есть ровно тот совет, ради удаления которого клауза и написана.
Измерено через штатный поток `handoff` с одно-проектным контролем в том же раунде.

Отягчающее и поучительное: докстринг `_predecessor_frozen` утверждал обратное как измеренный
факт — «a frozen predecessor reads the same whether it was resolved on this board or on a
neighbour's, and no new key has to be threaded through `_offboard_predecessor`». Предложение
называло тот самый метод, который его опровергает, то есть кросс-проектная ветка была
рассмотрена и закрыта ЧТЕНИЕМ. Вторая ошибка того же вида, что и выше, в одной карточке.

Починка — решать заморожен ли предшественник ТАМ, где стадия ещё сырая (в
`_offboard_predecessor`), и отдавать вердикт ключом `frozen`, как уже отдаются `escape` и
`finishable`. Сравнение по отрендеренной строке работало бы сегодня и сломалось бы при первой
же переформулировке той f-строки.

### Потолок CLAUDE.md поднят 40 000 -> 40 652, и первая попытка сделала это неправильно

Файл стоял на 39 876 при потолке 40 000 — 124 символа запаса. Правило про восьмую стадию
заняло 652 (с учётом второго захода, добавившего гейт), потолок сдвинут на те же 652, так что
запас до и после — 124. Эта арифметика и есть всё обоснование.

**Первая версия подняла потолок на 500 против роста файла на 461** — то есть ВЫДАЛА 39
символов бюджета, ровно то движение «поднять, пока не влезет», которое шапка самого гейта
отвергает, — и описала результат как запас «МЕНЬШЕ прежнего», написав измеренную пару
`124 -> 163` в том же предложении. 163 больше 124. Поймано независимым ревью. Ошибка была не в
решении поднять (по существу оно защитимо: факт «REQUIRED_STAGES, а не STAGES» — это факт про
отказ всего флота, ему место в файле, который читает каждая сессия), а в том, что обоснование
не сходилось с собственными числами.

Заодно поправлена вторая измеренная ошибка того же комментария. Он утверждал, что буллет
`workflow.py` — самый длинный в разделе Architecture, и направлял туда следующий шаг ратчета.
Измерено на посадке: `workspace_cmd.py` 4 319, `config.py` 3 624, `workflow.py` 2 947 — третий.

Пара «символы/токены» пересчитана согласованно (3.15x/3.06x -> 3.10x/3.01x); токенная половина
— арифметика по тем же заякоренным per-character rates, а не новый прогон токенизатора, потому
что скрипт текста не менялся, менялся только потолок.

### Что заморозка стопорит МОЛЧА — и почему две из трёх дыр закрыты, а третья нет

Заморозка карточки останавливает три вещи, и это стоит держать вместе, потому что лечатся они
по-разному.

1. **Преемник за замороженным предшественником.** Закрыто клаузой `_predecessor_frozen` —
   печатается в отказе `claim` и в `_starving_tail`, то есть и тому, кто спросил карточку по id,
   и тому, кто просто дренит очередь.
2. **Шов между двумя неисполнимыми клаузами.** Найдено ревью: `_predecessor_escapes` не
   заканчивает свою фразу точкой, поэтому карточка, ждущая одновременно нечитаемого И
   замороженного предшественника, читалась как «...clears the gate outright 1 of those sit(s) in
   Icebox...» — два предложения, сваренных встык. Оба шва теперь спрашивают через
   `_sentence_join`, а не приклеивают пробелом.
3. **`epic-ready` у эпика с замороженным ребёнком не выставится НИКОГДА, и это молчит.**
   `_mark_epic_if_children_complete` требует, чтобы ВСЕ дети были в `READY_STAGES`, а Icebox
   туда не входит и не должен. Не падение и не потеря — маркер best-effort, — но это третье
   место, где заморозка что-то стопорит, и ЕДИНСТВЕННОЕ из трёх, которое молчит. Оставлено
   осознанно: сигнал здесь пришлось бы адресовать человеку, а не агенту (только человек двигает
   в Done), и канала для этого у контейнера нет. Если будете чинить — чините канал, а не
   `READY_STAGES`: замороженный ребёнок действительно не готов.
