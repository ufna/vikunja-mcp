...
def test_fake_remove_label_idempotent_and_mirrors_client(env):
    ...
    висящей на снапшоте) — разбор в докстринге `FakeAPI._read_task`. Симметричное расхождение по
    ADD закрыто в #1216: там оно было МАРШРУТОМ, а не гонкой, и фейк теперь отвечает измеренным
    400 (`test_workflow_duplicate_label.py`). Имя теста не переименовано,
